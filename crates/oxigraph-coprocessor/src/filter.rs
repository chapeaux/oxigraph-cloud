...
pub fn evaluate_filter(key: &[u8], predicate: &FilterPredicate) -> bool {
    match predicate {
        FilterPredicate::TermEquals { position, value } => {
            extract_term_at(key, *position) == Some(value.as_slice())
        }
        FilterPredicate::TermTypeEquals {
            position,
            type_byte,
        } => extract_term_at(key, *position).is_some_and(|t| !t.is_empty() && t[0] == *type_byte),
...
fn extract_term_at(key: &[u8], position: usize) -> Option<&[u8]> {
    ...
    for i in 0..=position {
        ...
        if i == position {
            return Some(&key[offset..offset + len]);
        }
        offset += len;
    }
    None
...
mod tests {
    ...
    fn test_term_type_equals() {
        let mut key = vec![1_u8]; // NamedNode type
        key.extend_from_slice(&[0xAA; 16]);
        let pred = FilterPredicate::TermTypeEquals {
            position: 0,
            type_byte: 1,
        };
        assert!(evaluate_filter(&key, &pred));
        let pred_wrong = FilterPredicate::TermTypeEquals {
            position: 0,
            type_byte: 8,
        };
    ...
    fn test_and_filter() {
        let mut key = Vec::new();
        key.push(1);
        key.extend_from_slice(&[0xAA; 16]); // NamedNode
