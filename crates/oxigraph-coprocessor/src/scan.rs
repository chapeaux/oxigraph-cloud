...
pub fn execute_index_scan<'a>(
    ...
    for (key, value) in pairs {
        ...
        if let Some(bloom_bytes) = &params.bloom_filter {
            let key_without_prefix = &key[1..];
            if extract_concat_term_bytes(key_without_prefix, &params.bloom_positions)
                .is_ok_and(|concat| !bloom_check(bloom_bytes, &concat))
            {
                continue;
            }
        }
        ...
        if params.limit > 0 && result.pairs.len() as u64 >= params.limit {
            break;
        }
    }
    result
...
fn bloom_check(filter: &[u8], key: &[u8]) -> bool {
    ...
    // Intentional truncation for bloom filter hashing
    #[expect(clippy::cast_possible_truncation)]
    let (h1, h2) = (h as usize, (h >> 32) as usize);
    for i in 0_usize..3 {
