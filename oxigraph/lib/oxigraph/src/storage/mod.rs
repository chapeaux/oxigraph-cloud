...
use crate::storage::tikv::{
    TiKvDecodingGraphIterator, TiKvDecodingQuadIterator, TiKvStorage, TiKvStorageBulkLoader,
    TiKvStorageReadableTransaction, TiKvStorageReader, TiKvStorageTransaction,
};
use oxrdf::Quad;
#[cfg(all(not(target_family = "wasm"), feature = "rocksdb"))]
use std::path::Path;
...
impl Storage {
    ...
    #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
    pub fn open_tikv(pd_endpoints: &[String]) -> Result<Self, StorageError> {
        Ok(Self {
            kind: StorageKind::TiKv(TiKvStorage::connect(pd_endpoints)?),
        })
    }
    ...
    pub fn snapshot(&self) -> StorageReader<'static> {
        StorageReader {
            kind: match &self.kind {
                ...
                #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
                StorageKind::TiKv(storage) => StorageReaderKind::TiKv(storage.snapshot()),
    ...
    pub fn start_transaction(&self) -> Result<StorageTransaction<'_>, StorageError> {
        Ok(StorageTransaction {
            kind: match &self.kind {
                ...
                #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
                StorageKind::TiKv(storage) => {
                    StorageTransactionKind::TiKv(storage.start_transaction()?)
                }
    ...
    pub fn start_readable_transaction(
        ...
        Ok(StorageReadableTransaction {
            kind: match &self.kind {
                ...
                #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
                StorageKind::TiKv(storage) => {
                    StorageReadableTransactionKind::TiKv(storage.start_readable_transaction()?)
                }
                StorageKind::Memory(storage) => {
    ...
    pub fn backup(&self, target_directory: &Path) -> Result<(), StorageError> {
        match &self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageKind::TiKv(_) => Err(StorageError::Other(
                "It is not possible to backup a TiKV database".into(),
            )),
    ...
    pub fn bulk_loader(&self) -> StorageBulkLoader<'_> {
        match &self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageKind::TiKv(storage) => StorageBulkLoader {
                kind: StorageBulkLoaderKind::TiKv(storage.bulk_loader()),
            },
...
impl<'a> StorageReader<'a> {
    pub fn len(&self) -> Result<usize, StorageError> {
        match &self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReaderKind::TiKv(reader) => reader.len(),
    ...
    pub fn is_empty(&self) -> Result<bool, StorageError> {
        match &self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReaderKind::TiKv(reader) => reader.is_empty(),
    ...
    pub fn contains(&self, quad: &EncodedQuad) -> Result<bool, StorageError> {
        match &self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReaderKind::TiKv(reader) => reader.contains(quad),
    ...
    pub fn quads_for_pattern(
        ...
        DecodingQuadIterator {
            kind: match &self.kind {
                ...
                #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
                StorageReaderKind::TiKv(reader) => DecodingQuadIteratorKind::TiKv(
                    reader.quads_for_pattern(subject, predicate, object, graph_name),
                ),
    ...
    pub fn named_graphs(&self) -> DecodingGraphIterator<'a> {
        DecodingGraphIterator {
            kind: match &self.kind {
                ...
                #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
                StorageReaderKind::TiKv(reader) => {
                    DecodingGraphIteratorKind::TiKv(reader.named_graphs())
                }
    ...
    pub fn contains_named_graph(&self, graph_name: &EncodedTerm) -> Result<bool, StorageError> {
        match &self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReaderKind::TiKv(reader) => reader.contains_named_graph(graph_name),
    ...
    pub fn contains_str(&self, key: &StrHash) -> Result<bool, StorageError> {
        match &self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReaderKind::TiKv(reader) => reader.contains_str(key),
    ...
    pub fn validate(&self) -> Result<(), StorageError> {
        match &self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReaderKind::TiKv(reader) => reader.validate(),
...
impl Iterator for DecodingQuadIterator<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            DecodingQuadIteratorKind::TiKv(iter) => iter.next(),
...
enum DecodingGraphIteratorKind<'a> {
    ...
    #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
    TiKv(TiKvDecodingGraphIterator),
...
impl Iterator for DecodingGraphIterator<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            DecodingGraphIteratorKind::TiKv(iter) => iter.next(),
...
impl StrLookup for StorageReader<'_> {
    fn get_str(&self, key: &StrHash) -> Result<Option<String>, StorageError> {
        match &self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReaderKind::TiKv(reader) => reader.get_str(key),
...
impl StorageTransaction<'_> {
    pub fn insert(&mut self, quad: QuadRef<'_>) {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageTransactionKind::TiKv(transaction) => transaction.insert(quad),
    ...
    pub fn remove(&mut self, quad: QuadRef<'_>) {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageTransactionKind::TiKv(transaction) => transaction.remove(quad),
    ...
    pub fn clear_default_graph(&mut self) {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageTransactionKind::TiKv(transaction) => transaction.clear_default_graph(),
            StorageTransactionKind::Memory(transaction) => {
    ...
    pub fn clear_all_named_graphs(&mut self) {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageTransactionKind::TiKv(transaction) => transaction.clear_all_named_graphs(),
    ...
    pub fn clear_all_graphs(&mut self) {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageTransactionKind::TiKv(transaction) => transaction.clear_all_graphs(),
    ...
    pub fn remove_all_named_graphs(&mut self) {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageTransactionKind::TiKv(transaction) => transaction.remove_all_named_graphs(),
    ...
    pub fn clear(&mut self) {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageTransactionKind::TiKv(transaction) => transaction.clear(),
    ...
    pub fn commit(self) -> Result<(), StorageError> {
        match self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageTransactionKind::TiKv(transaction) => transaction.commit(),
            StorageTransactionKind::Memory(transaction) => {
                transaction.commit();
                Ok(())
            }
...
enum StorageReadableTransactionKind<'a> {
    ...
    #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
    TiKv(TiKvStorageReadableTransaction<'a>),
    Memory(MemoryStorageTransaction<'a>),
}
...
impl StorageReadableTransaction<'_> {
    pub fn reader(&self) -> StorageReader<'_> {
        StorageReader {
            kind: match &self.kind {
                ...
                #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
                StorageReadableTransactionKind::TiKv(transaction) => {
                    StorageReaderKind::TiKv(transaction.reader())
                }
    ...
    pub fn insert(&mut self, quad: QuadRef<'_>) {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReadableTransactionKind::TiKv(transaction) => transaction.insert(quad),
            StorageReadableTransactionKind::Memory(transaction) => {
                transaction.insert(quad);
            }
        }
    ...
    pub fn remove(&mut self, quad: QuadRef<'_>) {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReadableTransactionKind::TiKv(transaction) => transaction.remove(quad),
    ...
    pub fn clear_all_graphs(&mut self) -> Result<(), StorageError> {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReadableTransactionKind::TiKv(transaction) => transaction.clear_all_graphs(),
            StorageReadableTransactionKind::Memory(transaction) => {
    ...
    pub fn clear(&mut self) -> Result<(), StorageError> {
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReadableTransactionKind::TiKv(transaction) => transaction.clear(),
            StorageReadableTransactionKind::Memory(transaction) => {
                transaction.clear();
                Ok(())
            }
    ...
    pub fn commit(self) -> Result<(), StorageError> {
        match self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageReadableTransactionKind::TiKv(transaction) => transaction.commit(),
            StorageReadableTransactionKind::Memory(transaction) => {
                transaction.commit();
                Ok(())
            }
...
impl StorageBulkLoader<'_> {
    pub fn on_progress(self, callback: impl Fn(u64) + Send + Sync + 'static) -> Self {
        match self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageBulkLoaderKind::TiKv(loader) => Self {
                kind: StorageBulkLoaderKind::TiKv(loader.on_progress(callback)),
            },
    ...
    pub fn without_atomicity(self) -> Self {
        match self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageBulkLoaderKind::TiKv(loader) => Self {
                kind: StorageBulkLoaderKind::TiKv(loader.without_atomicity()),
            },
            StorageBulkLoaderKind::Memory(loader) => Self {
    ...
    pub fn load_batch(
        ...
        match &mut self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageBulkLoaderKind::TiKv(loader) => loader.load_batch(quads, max_num_threads),
    ...
    pub fn commit(self) -> Result<(), StorageError> {
        match self.kind {
            ...
            #[cfg(all(not(target_family = "wasm"), feature = "tikv"))]
            StorageBulkLoaderKind::TiKv(loader) => loader.commit(),
            StorageBulkLoaderKind::Memory(loader) => {
                loader.commit();
                Ok(())
            }
